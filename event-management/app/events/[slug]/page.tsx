    ...
    <div>
      ...
      <div>
        ...
        <p className="text-gray-300">
          At {event.time} on {new Date(event.date).toLocaleDateString()}
        </p>
        ...
        <h3 className="text-lg font-medium text-yellow mt-4">
          By {event.organizer}
        </h3>
        <div className="mt-4">
          <span>
            {event.tags.map((tag: string) => (
              <span
                key={tag}
                className="text-sm text-gray-300 mr-2 bg-gray-600 px-2 py-1 rounded"
              >
                {tag}
              </span>
            ))}
...
function EventLoading() {
  return <p className="text-center text-white mt-10">Loading event...</p>;
}
